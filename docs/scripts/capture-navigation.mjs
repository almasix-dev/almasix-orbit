/**
 * Capture Navigation gallery shots (light + dark).
 * Usage: node docs/scripts/capture-navigation.mjs
 */
...
async function main() {
  ...
  for (const theme of ["light", "dark"]) {
    ...
    const shots = page.locator('[data-shot^="navigation/"]');
    const count = await shots.count();
    if (!count) {
      throw new Error("No navigation/[data-shot] frames found — rebuild gallery");
    }
