/**
 * Capture Tenancy gallery shots (light + dark).
 * Usage: node docs/scripts/capture-tenancy.mjs
 */
...
async function main() {
  ...
  for (const theme of ["light", "dark"]) {
    ...
    const shots = page.locator('[data-shot^="users/tenancy/"]');
    const count = await shots.count();
    if (!count) {
      throw new Error("No users/tenancy/[data-shot] frames found — rebuild gallery");
    }
